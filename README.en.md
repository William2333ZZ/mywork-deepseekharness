...
- Live browser: twice as sharp on Retina. The background Chrome only screencasts at 1x, so as soon as the page is still a crisp frame is captured at the screen's pixel ratio (webp); motion keeps using the live stream. A status pill over the view says "the model is browsing · click the page to take over" right after the model navigates; any click or scroll hands control to you and stops following, "Follow the model" resumes.
