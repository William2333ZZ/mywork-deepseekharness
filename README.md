...
> 微信个人号机器人只能在你给它发过消息后的一段时间内回复（平台限制，见「已知的坑」）。飞书账号配置好就能直接推送，不用先发消息。要准点推送，把通知目标选成飞书 / 钉钉 / 企业微信账号。
...
- 定时任务里 Automation 的开关不再是固定绿色，打开时跟随当前风格的主题色。
- 飞书通知不再要求先给机器人发消息：配置好飞书账号后，「频道」标签立刻显示该账号，定时任务的「运行结束发到 IM」和 `im_send` 里多出一个「飞书账号 1 · 直接推送给我」目标，通过飞书机器人 API 直接发给账号所有者。微信仍受平台限制。
- 实时浏览器面板改成 Claude 桌面版浏览器的样式：胶囊形标签条（每个标签可关、可加）、更宽的圆角地址栏、地址栏旁的「跟随 / 接管」按钮、主题色的空状态页；面板很窄时次要按钮自动收起。
