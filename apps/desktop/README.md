...
```
node apps/desktop/build.mjs win-x64     # → dist/MyWork-DSH-win-x64.zip（免安装，解压双击）
node apps/desktop/build.mjs mac-arm64   # → dist/mac-arm64/MyWork DSH.app
bash apps/desktop/package-mac.sh        # → dist/MyWork-DSH-mac-arm64.dmg
```
...
数据目录：Windows `%APPDATA%\MyWork DSH\`，macOS `~/Library/Application Support/MyWork DSH/`；里面是 `home/`（DSH_HOME）和 `dsh.log`。
